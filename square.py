...
class Square(point.Point):
    ...
    def __init__(self, pos: Tuple[float, float], size: int, board_top_left: Tuple[int, int]) -> None:
        super().__init__(pos)
    ...
    def _set_rect(self):
        self.rect.left = self.board_top_left[1] + (self.size * (self.col() - 1))
        self.rect.top = self.board_top_left[0] + (self.size * (self.row() - 1))
